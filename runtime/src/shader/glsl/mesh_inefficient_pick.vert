...
layout(set = 0, binding = 1) readonly buffer _unused_name_perdrawcall
{
    ...
    int enable_vertex_blendings[m_mesh_per_drawcall_max_instance_count];
};
...
void main()
{
    ...
    int enable_vertex_blending = enable_vertex_blendings[gl_InstanceIndex];

    highp vec3 model_position;
    if (enable_vertex_blending > 0)
    {
