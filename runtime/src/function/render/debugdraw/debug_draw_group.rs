...
use crate::{
    core::math::{
        matrix4::{Matrix4x4, ToScaleMatrix4x4},
        quaternion::Quaternion,
        vector2::Vector2,
        vector3::Vector3,
        vector4::Vector4,
    },
    function::render::debugdraw::{
        debug_draw_font::DebugDrawFont,
        debug_draw_primitive::{
            DebugDrawBox, DebugDrawCapsule, DebugDrawCylinder, DebugDrawLine, DebugDrawPoint,
            DebugDrawQuad, DebugDrawSphere, DebugDrawText, DebugDrawTriangle, DebugDrawVertex,
            FillMode,
        },
    },
};
...
pub struct DebugDrawGroup {
    ...
    m_triangles: Vec<DebugDrawTriangle>,
...
impl DebugDrawGroup {
    ...
    pub fn remove_dead_primitives(&mut self, delta_time: f32) {
        self.m_points
            .retain_mut(|p: &mut DebugDrawPoint| !p.m_base.is_time_out(delta_time));
        self.m_lines
            .retain_mut(|l: &mut DebugDrawLine| !l.m_base.is_time_out(delta_time));
        self.m_triangles
            .retain_mut(|t: &mut DebugDrawTriangle| !t.m_base.is_time_out(delta_time));
        self.m_quads
            .retain_mut(|q: &mut DebugDrawQuad| !q.m_base.is_time_out(delta_time));
        self.m_boxes
            .retain_mut(|b: &mut DebugDrawBox| !b.m_base.is_time_out(delta_time));
        self.m_cylinders
            .retain_mut(|c: &mut DebugDrawCylinder| !c.m_base.is_time_out(delta_time));
        self.m_spheres
            .retain_mut(|s: &mut DebugDrawSphere| !s.m_base.is_time_out(delta_time));
        self.m_capsules
            .retain_mut(|c: &mut DebugDrawCapsule| !c.m_base.is_time_out(delta_time));
    ...
    pub fn get_line_count(&self, no_depth_test: bool) -> usize {
        ...
        res += self
            .m_lines
            .iter()
            .filter(|l| l.m_base.m_no_depth_test == no_depth_test)
            .count();
        res += self
            .m_triangles
            .iter()
            .filter(|t| {
                t.m_base.m_fill_mode == FillMode::WireFrame
                    && t.m_base.m_no_depth_test == no_depth_test
            })
            .count()
            * 3;
        res += self
            ...
            .iter()
            .filter(|q| {
                q.m_base.m_fill_mode == FillMode::WireFrame
                    && q.m_base.m_no_depth_test == no_depth_test
            })
            .count()
            * 4;
        res += self
            ...
            .iter()
            .filter(|b| {
                b.m_base.m_fill_mode == FillMode::WireFrame
                    && b.m_base.m_no_depth_test == no_depth_test
            })
            .count()
            * 12;
        res
    ...
    pub fn write_line_data(&self, no_depth_test: bool) -> Vec<DebugDrawVertex> {
        ...
        res.extend(
            self.m_lines
                .iter()
                .filter(|l| {
                    l.m_base.m_fill_mode == FillMode::WireFrame
                        && l.m_base.m_no_depth_test == no_depth_test
                })
    ...
    pub fn write_triangle_data(&self, no_depth_test: bool) -> Vec<DebugDrawVertex> {
        self.m_triangles
            ...
            .flat_map(|t| t.m_vertex)
            .collect()
    }

    pub fn write_text_data(
        &self,
        _font: &DebugDrawFont,
        ...
        for text in &self.m_texts {
            ...
            if !text.m_is_screen_text {
                ...
                coordinate = Vector3::new(
                    temp_coord.x / temp_coord.w,
                    temp_coord.y / temp_coord.w,
                    0.0,
                );
            }
            ...
            for character in text.m_content.chars() {
                if character == '\n' {
                    ...
                    let (x1, x2, y1, y2) =
                        DebugDrawFont::get_character_texture_rect(character as u8);
    ...
    pub fn write_uniform_dynamic_data_to_cache(&self) -> Vec<(Matrix4x4, Vector4)> {
        ...
        for i in 0..2 {
            ...
            res.extend(
                self.m_spheres
                    .iter()
                    .filter(|obj| obj.m_base.m_no_depth_test == no_depth_test)
                    .map(|obj| {
            ...
            res.extend(
                self.m_cylinders
                    ...
                    .map(|obj| {
                        let trans = obj.m_center.to_translate_matrix();
                        let scale = Vector3::new(obj.m_radius, obj.m_radius, obj.m_height * 0.5)
                            .to_scale_matrix();
            ...
            res.extend(
                self.m_capsules
                    ...
                    .flat_map(|obj| {
                        ...
                        let rotate = obj.m_rotate.to_rotation_matrix();
                        let trans1 = Vector3::new(0.0, 0.0, obj.m_height * 0.5 - obj.m_radius)
                            .to_translate_matrix();
                        let scale2 = Vector3::new(1.0, 1.0, obj.m_height / (obj.m_radius * 2.0))
                            .to_scale_matrix();
                        let trans3 = Vector3::new(0.0, 0.0, -(obj.m_height * 0.5 - obj.m_radius))
                            .to_translate_matrix();
                        [
    ...
    pub fn get_sphere_count(&self, no_depth_test: bool) -> usize {
        self.m_spheres
            .iter()
            .filter(|s| s.m_base.m_no_depth_test == no_depth_test)
            .count()
    }
    ...
    pub fn get_cylinder_count(&self, no_depth_test: bool) -> usize {
        self.m_cylinders
            .iter()
            .filter(|c| c.m_base.m_no_depth_test == no_depth_test)
            .count()
    }
    ...
    pub fn get_text_character_count(&self) -> usize {
        self.m_texts
            .iter()
            .map(|t| t.m_content.chars().filter(|c| *c != '\n').count())
            .sum()
    }
}
