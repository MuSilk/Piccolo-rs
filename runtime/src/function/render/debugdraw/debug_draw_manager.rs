...
use crate::{
    ...
    function::render::{
        debugdraw::{
            debug_draw_buffer::DebugDrawAllocator,
            debug_draw_context::DebugDrawContext,
            debug_draw_font::DebugDrawFont,
            debug_draw_group::DebugDrawGroup,
            debug_draw_pipeline::{DebugDrawPipeline, DebugDrawPipelineType},
        },
...
impl DebugDrawManager {
    pub fn create(info: &DebugDrawManagerCreateInfo) -> Result<Self> {
        ...
        let m_debug_draw_pipelines = [
            DebugDrawPipeline::create(DebugDrawPipelineType::Point, info.rhi, set_layout)?,
            DebugDrawPipeline::create(DebugDrawPipelineType::Line, info.rhi, set_layout)?,
            DebugDrawPipeline::create(DebugDrawPipelineType::Triangle, info.rhi, set_layout)?,
            DebugDrawPipeline::create(
            ...
            DebugDrawPipeline::create(
                DebugDrawPipelineType::TriangleNoDepthTest,
                info.rhi,
                set_layout,
            )?,
        ];
    ...
    pub fn prepare_pass_data(&mut self, render_resource: &RenderResource) {
        self.m_proj_view_matrix = render_resource
...
impl DebugDrawManager {
    fn swap_data_to_render(&mut self) {
        ...
        self.m_debug_context
            ...
            .iter()
            .for_each(|group| {
                self.m_debug_draw_group_for_render
                    .merge_from(&group.borrow());
            });
    }
    ...
    fn prepare_draw_buffer(&mut self) -> Result<()> {
        ...
        let vertices = self
            .m_debug_draw_group_for_render
            .write_triangle_data(false);
        ...
        self.m_base.m_no_depth_test_point_start_offset =
            self.m_buffer_allocator.cache_vertices(&vertices);
        ...
        let vertices = self.m_debug_draw_group_for_render.write_text_data(
            &self.m_font,
            &self.m_proj_view_matrix,
            screen_width,
            screen_height,
        );
        ...
        self.m_buffer_allocator
            .cache_uniform_object(self.m_proj_view_matrix);

        let dynamic_objects = self
            .m_debug_draw_group_for_render
            .write_uniform_dynamic_data_to_cache();
        self.m_buffer_allocator
    ...
    fn draw_point_line_triangle_box(&self, current_swapchain_image_index: usize) {
        ...
        for i in 0..vc_pipelines.len() {
            ...
            rhi.cmd_draw(
                command_buffer,
                (vc_end_offsets[i] - vc_start_offsets[i]) as u32,
                1,
                vc_start_offsets[i] as u32,
                0,
            );
    ...
    fn draw_wire_frame_object(&mut self, current_swapchain_image_index: usize) -> Result<()> {
        ...
        let uniform_dynamic_size =
            DebugDrawAllocator::get_size_of_uniform_buffer_dynamic_object() as u32;
        ...
        for i in 0..vc_pipelines.len() {
            ...
            let sphere_count = self
                .m_debug_draw_group_for_render
                .get_sphere_count(no_depth_test);
            if sphere_count > 0 {
                for _i in 0..sphere_count {
                    ...
                    rhi.cmd_draw(
                        command_buffer,
                        DebugDrawAllocator::get_sphere_vertex_buffer_size() as u32,
                        1,
                        0,
                        0,
                    );
            ...
            let cylinder_count = self
                .m_debug_draw_group_for_render
                .get_cylinder_count(no_depth_test);
            if cylinder_count > 0 {
                for _i in 0..cylinder_count {
                    ...
                    rhi.cmd_draw(
                        command_buffer,
                        DebugDrawAllocator::get_cylinder_vertex_buffer_size() as u32,
                        1,
                        0,
                        0,
                    );
            ...
            let capsule_count = self
                .m_debug_draw_group_for_render
                .get_capsule_count(no_depth_test);
            if capsule_count > 0 {
                for _i in 0..capsule_count {
                    ...
                    rhi.cmd_draw(
                        command_buffer,
                        DebugDrawAllocator::get_capsule_vertex_buffer_up_size() as u32,
                        1,
                        0,
                        0,
                    );
                    ...
                    rhi.cmd_draw(
                        command_buffer,
                        DebugDrawAllocator::get_capsule_vertex_buffer_mid_size() as u32,
                        1,
                        0,
                        0,
                    );
                    ...
                    rhi.cmd_draw(
                        command_buffer,
                        DebugDrawAllocator::get_capsule_vertex_buffer_down_size() as u32,
                        1,
                        0,
                        0,
                    );
