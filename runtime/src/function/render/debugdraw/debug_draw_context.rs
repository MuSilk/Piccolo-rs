...
use crate::function::render::debugdraw::debug_draw_group::DebugDrawGroup;
...
impl DebugDrawContext {
    pub fn try_get_or_create_debug_draw_group(&mut self, name: &str) -> &RefCell<DebugDrawGroup> {
        ...
        if self.m_debug_draw_groups.iter().find(|g| g.borrow().get_name() == name).is_none() {
            let mut debug_draw_group = DebugDrawGroup::create();
            debug_draw_group.set_name(name);
            self.m_debug_draw_groups.push(RefCell::new(debug_draw_group));
        }
        return &self.m_debug_draw_groups.iter().find(|g| g.borrow().get_name() == name).unwrap();
    }
