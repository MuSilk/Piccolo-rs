...
use crate::{
    ...
    shader::generated::shader::{FXAA_FRAG, FXAA_VERT},
};
...
pub struct FXAADescriptorLayout;
impl DescriptorLayout for FXAADescriptorLayout {
...
impl FXAAPass {
    fn setup_descriptor_layout(
        ...
        let layout = descriptor_layout_manager.acquire::<FXAADescriptorLayout>(rhi)?;
    ...
    fn setup_pipelines(&mut self, rhi: &VulkanRHI) -> Result<()> {
        ...
        let vert_shader_module = rhi.create_shader_module(&FXAA_VERT)?;
        let frag_shader_module = rhi.create_shader_module(&FXAA_FRAG)?;
