...
use crate::function::render::{
    interface::rhi_struct::{
        RHIBuffer, RHICommandBuffer, RHIDepthImageDesc, RHIDescriptorPool, RHIDescriptorSet,
        RHIDescriptorSetAllocateInfo, RHIDescriptorSetLayout, RHIDescriptorSetLayoutCreateInfo,
        RHIDeviceMemory, RHIFramebuffer, RHIFramebufferCreateInfo, RHIGraphicsPipelineCreateInfo,
        RHIImage, RHIImageView, RHIPipeline, RHIPipelineLayout, RHIPipelineLayoutCreateInfo,
        RHIRect2D, RHIRenderPass, RHIRenderPassBeginInfo, RHIRenderPassCreateInfo, RHISampler,
        RHIShader, RHISwapChainDesc, RHIViewport, RHIWriteDescriptorSet,
    },
    render_type::{
        RHIBufferUsageFlags, RHIDeviceSize, RHIFormat, RHIMemoryMapFlags, RHIMemoryPropertyFlags,
        RHIPipelineBindPoint, RHISamplerType, RHISubpassContents,
    },
...
pub trait RHI: Any + Send + Sync {
    ...
    fn allocate_descriptor_sets(
        &self,
        allocate_info: &RHIDescriptorSetAllocateInfo,
    ) -> Result<Vec<Box<dyn RHIDescriptorSet>>>;
    ...
    ) -> Result<&Box<dyn RHISampler>>;
    fn create_shader_module(&self, data: &[u8]) -> Result<Box<dyn RHIShader>>;
    fn create_buffer(
        &self,
        size: RHIDeviceSize,
        usage: RHIBufferUsageFlags,
        properties: RHIMemoryPropertyFlags,
    ) -> Result<(Box<dyn RHIBuffer>, Box<dyn RHIDeviceMemory>)>;
    fn copy_buffer(
        &self,
        src: &Box<dyn RHIBuffer>,
        dst: &Box<dyn RHIBuffer>,
        src_offset: RHIDeviceSize,
        dst_offset: RHIDeviceSize,
        size: RHIDeviceSize,
    ) -> Result<()>;
    fn create_texture_image(
        ...
        Box<dyn RHIImage>,
        Box<dyn RHIDeviceMemory>,
        Box<dyn RHIImageView>,
    )>;
    fn create_descriptor_set_layout(
        &self,
        create_info: &RHIDescriptorSetLayoutCreateInfo,
    ) -> Result<Box<dyn RHIDescriptorSetLayout>>;
    fn create_framebuffer(
        &self,
        create_info: &RHIFramebufferCreateInfo,
    ) -> Result<Box<dyn RHIFramebuffer>>;
    fn create_graphics_pipelines(
        &self,
        create_info: &[RHIGraphicsPipelineCreateInfo],
    ) -> Result<Vec<Box<dyn RHIPipeline>>>;
    fn create_pipeline_layout(
        &self,
        create_info: &RHIPipelineLayoutCreateInfo,
    ) -> Result<Box<dyn RHIPipelineLayout>>;
    fn create_render_pass(
        &self,
        create_info: &RHIRenderPassCreateInfo,
    ) -> Result<Box<dyn RHIRenderPass>>;

    // command and command write
    fn cmd_begin_render_pass(
        &self,
        command_buffer: &Box<dyn RHICommandBuffer>,
        render_pass_begin_info: &RHIRenderPassBeginInfo,
        contents: RHISubpassContents,
    );
    ...
    fn cmd_bind_vertex_buffers(
        ...
        command_buffer: &Box<dyn RHICommandBuffer>,
        first_binding: u32,
        buffers: &[&Box<dyn RHIBuffer>],
        offsets: &[RHIDeviceSize],
    );
    fn cmd_bind_descriptor_sets(
        ...
        first_set: u32,
        descriptor_sets: &[&Box<dyn RHIDescriptorSet>],
        dynamic_offsets: &[u32],
    );
    ...
    fn prepare_before_pass(
        &mut self,
        pass_update_after_recreate_swapchain: &dyn Fn(),
    ) -> Result<bool>;
