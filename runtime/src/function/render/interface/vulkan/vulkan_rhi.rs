...
use crate::function::render::{
    interface::{
        rhi::RHICreateInfo,
        rhi_struct::{
            QueueFamilyIndices, RHIDepthImageDesc, RHISwapChainDesc, SwapChainSupportDetails,
        },
        vulkan::vulkan_util::{self, create_image_view},
    },
...
impl VulkanRHI {
    ...
    pub fn destroy(&mut self) {
        unsafe {
            ...
            self.m_device
                .destroy_image_view(self.m_data.m_depth_image_view, None);
            self.m_device.destroy_image(self.m_data.m_depth_image, None);
            self.m_device
                .free_memory(self.m_data.m_depth_image_memory, None);

            self.m_data
                .m_swapchain_image_views
                .iter()
                .for_each(|v| self.m_device.destroy_image_view(*v, None));
            self.m_device
                .destroy_swapchain_khr(self.m_data.m_swapchain, None);
            self.m_device
                .destroy_descriptor_pool(self.m_data.m_descriptor_pool, None);
            self.m_data
                .m_is_frame_in_flight_fences
                .iter()
                .for_each(|f| self.m_device.destroy_fence(*f, None));
            self.m_data
                .m_image_finished_for_presentation_semaphores
                .iter()
                .for_each(|s| self.m_device.destroy_semaphore(*s, None));
            self.m_data
                .m_image_available_for_render_semaphores
                .iter()
                .for_each(|s| self.m_device.destroy_semaphore(*s, None));
            self.m_data
                .m_command_pools
                .iter()
                .for_each(|p| self.m_device.destroy_command_pool(*p, None));
            self.m_device
                .destroy_command_pool(self.m_data.m_command_pool, None);
            self.m_device.destroy_device(None);
            self.m_instance
                .destroy_surface_khr(self.m_data.m_surface, None);

            if self.m_data.m_enable_validation_layers {
                self.m_instance
                    .destroy_debug_utils_messenger_ext(self.m_data.m_debug_messenger, None);
            }
    ...
    pub fn prepare_context(&mut self) {
        let command_buffer: vk::CommandBuffer =
            self.m_data.m_command_buffers[self.m_current_frame_index];
    ...
    pub fn allocate_descriptor_sets(
        ...
        let descriptor_sets = unsafe { self.m_device.allocate_descriptor_sets(&allocate_info)? };
        Ok(descriptor_sets)
    }
    ...
    fn recreate_swapchain(&mut self, window: &Window) -> Result<()> {
        unsafe {
            ...
            self.m_data
                .m_images_in_flight
                .resize(self.m_data.m_swapchain_images.len(), vk::Fence::null());
        }
        Ok(())
    }

    pub fn get_or_create_default_sampler(
        &self,
        sampler_type: RHISamplerType,
    ) -> Result<&vk::Sampler> {
        match sampler_type {
            RHISamplerType::Linear => Ok(self.m_data.m_linear_sampler.get_or_init(|| {
                let properties = unsafe {
                    self.m_instance
                        .get_physical_device_properties(self.m_data.m_physical_device)
                };
                ...
                unsafe { self.m_device.create_sampler(&create_info, None).unwrap() }
            })),
            RHISamplerType::Nearest => Ok(self.m_data.m_nearest_sampler.get_or_init(|| {
                let properties = unsafe {
                    self.m_instance
                        .get_physical_device_properties(self.m_data.m_physical_device)
                };
    ...
    pub fn get_or_create_mipmap_sampler(
        &self,
        width: u32,
        height: u32,
        sampler_type: RHISamplerType,
        ...
        if let Some(sampler) = self
            .m_data
            .m_mipmap_sampler_map
            .borrow()
            .get(&(mip_levels, sampler_type))
        {
            return Ok(*sampler);
        }
        ...
        let sampler = unsafe { self.m_device.create_sampler(&sampler_info, None)? };

        self.m_data
            .m_mipmap_sampler_map
            .borrow_mut()
            .insert((mip_levels, sampler_type), sampler);
        Ok(sampler)
    }
    ...
    pub fn create_cube_map(
        ...
        Ok(vulkan_util::create_cube_map(
            self, width, height, pixels, format, mip_levels,
        )?)
    }
    ...
    pub fn create_image_view(
        ...
        Ok(vulkan_util::create_image_view(
            &self.m_device,
            image,
            format,
            aspect_flags,
    ...
    pub fn create_framebuffer(
        ...
        Ok(unsafe { self.m_device.create_framebuffer(create_info, None)? })
    }
    ...
    pub fn cmd_begin_render_pass(
        ...
        unsafe {
            self.m_device.cmd_begin_render_pass(
                command_buffer,
                &begin_info,
                vk::SubpassContents::from_raw(contents.as_raw()),
            );
        }
    }
    ...
    pub fn cmd_push_constants(
        ...
        unsafe {
            self.m_device
                .cmd_push_constants(command_buffer, layout, stage_flags, offset, values);
        }
    }
    ...
    pub fn cmd_clear_attachments(
        &self,
        command_buffer: vk::CommandBuffer,
        clear_attachments: &[vk::ClearAttachment],
        clear_rects: &[vk::ClearRect],
    ) {
        unsafe {
            self.m_device
                .cmd_clear_attachments(command_buffer, clear_attachments, clear_rects);
        }
    }
    ...
    pub fn reset_command_pool(&self) -> Result<()> {
        unsafe {
            self.m_device.reset_command_pool(
                self.m_data.m_command_pools[self.m_current_frame_index],
    ...
    pub fn wait_for_fence(&self) -> Result<()> {
        unsafe {
            self.m_device.wait_for_fences(
                &[self.m_data.m_is_frame_in_flight_fences[self.m_current_frame_index]],
                true,
                std::u64::MAX,
            )?;
        }
    ...
    pub fn end_single_time_commands(&self, command_buffer: vk::CommandBuffer) -> Result<()> {
        ...
        unsafe {
            ...
            self.m_device
                .free_command_buffers(self.m_data.m_command_pool, &command_buffers);
        }
        Ok(())
    }
    ...
    pub fn prepare_before_pass(
        ...
        let image_in_flight =
            self.m_data.m_images_in_flight[self.m_data.m_current_swapchain_image_index];
        ...
        unsafe {
            self.m_device
                .begin_command_buffer(self.m_data.m_current_command_buffer, &info)?;
        }
    ...
    pub fn submit_rendering(
        ...
        let signal_semaphores = &[
            self.m_data.m_image_finished_for_presentation_semaphores[self.m_current_frame_index]
        ];
        ...
        unsafe {
            self.m_device.queue_submit(
                self.m_data.m_graphics_queue,
                &[submit_info],
                in_flight_fence,
            )
        }?;
        ...
        let result = unsafe {
            self.m_device
                .queue_present_khr(self.m_data.m_present_queue, &present_info)
        };
    ...
    pub fn map_memory(
        ...
        Ok(unsafe { self.m_device.map_memory(memory, offset, size, flags)? })
    }
...
fn check_validation_layer_support(entry: &Entry, data: &VulkanRHIData) -> Result<bool> {
    ...
    let res = data
        .m_validation_layers
        .iter()
        .all(|&layer_name| available_layers.contains(&layer_name));
...
fn populate_debug_messenger_info(
    info: vk::DebugUtilsMessengerCreateInfoEXTBuilder,
) -> vk::DebugUtilsMessengerCreateInfoEXTBuilder {
    info.message_severity(vk::DebugUtilsMessageSeverityFlagsEXT::all())
        .message_type(
    ...
    unsafe {
        Ok(SwapChainSupportDetails {
            capabilities: instance
                .get_physical_device_surface_capabilities_khr(physical_device, data.m_surface)?,
            formats: instance
                .get_physical_device_surface_formats_khr(physical_device, data.m_surface)?,
            present_modes: instance
                .get_physical_device_surface_present_modes_khr(physical_device, data.m_surface)?,
        })
    }
}
...
fn is_device_suitable(
    ...
    let is_swapchain_adequate = !swapchain_support_details.formats.is_empty()
...
fn create_logical_device(
    ...
    let device = unsafe { instance.create_device(data.m_physical_device, &device_info, None)? };

    unsafe {
        let vk_graphics_queue =
            device.get_device_queue(data.m_queue_indices.graphics_family.unwrap(), 0);
        data.m_graphics_queue = vk_graphics_queue;
        data.m_present_queue =
            device.get_device_queue(data.m_queue_indices.present_family.unwrap(), 0);
    }
...
fn create_descriptor_pool(device: &Device, data: &mut VulkanRHIData) -> Result<()> {
    ...
    .into_iter()
    .filter(|poolsize| poolsize.descriptor_count > 0)
    .collect::<Vec<_>>();
...
fn create_sync_objects(device: &Device, data: &mut VulkanRHIData) -> Result<()> {
    ...
    for i in 0..K_MAX_FRAMES_IN_FLIGHT {
        unsafe {
            data.m_image_available_for_render_semaphores[i] =
                device.create_semaphore(&semaphore_info, None)?;
            data.m_image_finished_for_presentation_semaphores[i] =
                device.create_semaphore(&semaphore_info, None)?;
...
fn create_swapchain(
    ...
    let swapchain_support_details =
        query_swapchain_support(instance, data, data.m_physical_device)?;
    ...
    let image_sharing_mode = if data.m_queue_indices.graphics_family.unwrap()
        != data.m_queue_indices.present_family.unwrap()
    {
...
fn create_swapchain_image_views(device: &Device, data: &mut VulkanRHIData) -> Result<()> {
    data.m_swapchain_image_views = data
        .m_swapchain_images
        .iter()
        .map(|i| {
            create_image_view(
