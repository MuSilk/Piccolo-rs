...
use crate::{function::{framework::{object::object_id_allocator::GObjectID, resource::component::mesh::MeshComponentRes}, render::render_object::{GameObjectLazyMeshDesc, GameObjectMeshDesc, GameObjectPartDesc}}, resource::{asset_manager::AssetManager, res_type::data::material::MaterialRes}};
        ...
        for (raw_mesh_index, sub_mesh) in mesh_res.m_sub_meshs.iter().enumerate() {
            ...
            if let Some(mesh_file) = &sub_mesh.m_obj_file_ref {
                mesh_component.m_mesh_desc = GameObjectMeshDesc::LazyMesh(
                    GameObjectLazyMeshDesc::new(asset_manager.get_full_path(mesh_file).to_str().unwrap().to_string())
                );
            }
            else{
