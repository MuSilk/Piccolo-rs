...
impl ApplicationHandler for WinitApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        self.app = unsafe { Some(App::create(&window).unwrap()) };
        ...
        match event {
            WindowEvent::RedrawRequested if !event_loop.exiting() &&!self.minimized => {
                unsafe{
                    ...
                    self.app.as_mut().unwrap().render(window).unwrap_or_else(|e|{error!("{}",e)});
                }
            }
...
impl App {
    ...
    unsafe fn create(window: &Window) -> Result<Self> {
        ...
        let mut data = VulkanRHI::default();
...
unsafe fn create_instance(window: &Window, entry: &Entry, data: &mut VulkanRHI) -> Result<Instance> {
    // Application Info
...
unsafe fn pick_physical_device(instance: &Instance, data: &mut VulkanRHI) -> Result<()> {
...
unsafe fn create_logical_device(entry: &Entry, instance: &Instance, data: &mut VulkanRHI) -> Result<Device> {
    // Queue Create Infos
...
unsafe fn create_swapchain(window: &Window, instance: &Instance, device: &Device, data: &mut VulkanRHI) -> Result<()> {
    // Image
...
unsafe fn create_swapchain_image_views(device: &Device, data: &mut VulkanRHI) -> Result<()> {
...
unsafe fn create_render_pass(instance: &Instance, device: &Device, data: &mut VulkanRHI) -> Result<()> {
    // Attachments
...
unsafe fn create_pipeline(device: &Device, data: &mut VulkanRHI) -> Result<()> {
    // Stages
...
unsafe fn create_framebuffers(device: &Device, data: &mut VulkanRHI) -> Result<()> {
...
unsafe fn create_command_pool(instance: &Instance, device: &Device, data: &mut VulkanRHI) -> Result<()> {
    let indices = QueueFamilyIndices::get(instance, data, data.physical_device)?;
...
unsafe fn create_command_buffers(device: &Device, data: &mut VulkanRHI) -> Result<()> {
    // Allocate
...
unsafe fn create_sync_objects(device: &Device, data: &mut VulkanRHI) -> Result<()> {
...
impl QueueFamilyIndices {
    unsafe fn get(instance: &Instance, data: &VulkanRHI, physical_device: vk::PhysicalDevice) -> Result<Self> {
