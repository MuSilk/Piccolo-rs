...
impl Editor {
    pub fn initialize(&mut self, engine: &Rc<RefCell<Engine>>) -> Result<()> {
        ...
        let ui_init_info = WindowUIInitInfo{
            window_system: &global.m_window_system.borrow(),
            render_system: &global.m_render_system.borrow(),
        };
...
impl ApplicationHandler for Editor {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let _ = global.m_window_system.borrow_mut().initialize(event_loop, window_create_info);
        let render_init_info = RenderSystemInitInfo{
            window_system: &global.m_window_system.borrow(),
        };
        let _ = global.m_render_system.borrow_mut().initialize(render_init_info);
        let _ = global.m_debugdraw_manager.borrow_mut().initialize();
    }

    fn window_event(&mut self, event_loop: &ActiveEventLoop, _window_id: WindowId, event: WindowEvent) {
        match event {
            WindowEvent::RedrawRequested if !event_loop.exiting() => {
                if !RuntimeGlobalContext::isinitialized() {
                    return;
                }
