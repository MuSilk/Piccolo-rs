...
impl DebugDrawManager {
    ...
    fn draw_point_line_triangle_box(&self, current_swap_chain_image_index: usize){
        ...
        let clear_value = [
            RHIClearValue::Color(RHIClearColorValue{float32:[1.0, 0.0, 0.0, 1.0]}),
        ...
        for i in 0..vc_pipelines.len() {
            ...
            let render_pass_begin_info = RHIRenderPassBeginInfo{
                render_pass: render_pass.as_ref().unwrap().as_ref(),
                framebuffer: framebuffer.as_ref(),
                render_area: RHIRect2D{offset: RHIOffset2D{x: 0, y: 0}, extent: rhi.get_swapchain_info().extent},
