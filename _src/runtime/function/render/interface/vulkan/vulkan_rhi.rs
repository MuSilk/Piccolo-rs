use std::{array, cell::OnceCell, collections::{HashMap, HashSet}, ffi::CStr, mem::transmute, os::raw::c_void, rc::Weak};
...
pub struct VulkanRHI {
    ...
    pub m_current_frame_index: usize,
    ...
    pub m_current_swapchain_image_index: u32,
...
impl RHI for VulkanRHI {
    ...
    fn prepare_context(&mut self) {
        self.m_vk_current_command_buffer = self.m_vk_command_buffers[self.m_current_frame_index];
        self.m_current_command_buffer = Some(Box::new(VulkanCommandBuffer::new(self.m_vk_current_command_buffer)));
    }
    ...
    fn recreate_swapchain(&mut self) -> Result<()> {
        unsafe{
            self.m_device.as_ref().unwrap().wait_for_fences(
                &self.m_is_frame_in_flight_fences,true, u64::MAX
            )?;
            let depth_image_view = self.m_depth_image_view.take().unwrap();
            self.destroy_image_view(depth_image_view);
            let depth_image = self.m_depth_image.take().unwrap();
            self.destroy_image(depth_image);
            self.m_device.as_ref().unwrap().free_memory(self.m_depth_image_memory, None);

            let swapchain_image_views = std::mem::take(&mut self.m_swapchain_image_views);
            swapchain_image_views.into_iter().for_each(|image_view| {
                self.destroy_image_view(image_view);
            });
            self.m_device.as_ref().unwrap().destroy_swapchain_khr(self.m_swapchain, None);
    ...
    fn create_graphics_pipelines(&self, create_info:&[RHIGraphicsPipelineCreateInfo]) -> Result<Vec<Box<dyn RHIPipeline>>> {
        ...
        let stages = create_info.iter().map(|info|{
            info.stages.iter().map(|stage| {
        ...
        let vk_binding_descriptions = create_info.iter().map(|info|{
            info.vertex_input_state.vertex_binding_descriptions.iter().map(|binding| {
        ...
        let vk_attribute_descriptions = create_info.iter().map(|info|{
            info.vertex_input_state.vertex_attribute_descriptions.iter().map(|attribute| {
        ...
        let vertex_input_states = create_info.iter().enumerate().map(|(i, info)| {
            vk::PipelineVertexInputStateCreateInfo::builder()
                .flags(unsafe{vk::PipelineVertexInputStateCreateFlags::from_bits_unchecked(info.vertex_input_state.flags.bits())})
                .vertex_binding_descriptions(&vk_binding_descriptions[i])
                .vertex_attribute_descriptions(&vk_attribute_descriptions[i])
        }).collect::<Vec<_>>();

        let input_assembly_states =  create_info.iter().map(|info| {
            vk::PipelineInputAssemblyStateCreateInfo::builder()
                .flags(unsafe{vk::PipelineInputAssemblyStateCreateFlags::from_bits_unchecked(info.input_assembly_state.flags.bits())})
                .topology(vk::PrimitiveTopology::from_raw(info.input_assembly_state.topology.as_raw()))
                .primitive_restart_enable(info.input_assembly_state.primitive_restart_enable)
        }).collect::<Vec<_>>();

        //tessellation state

        let viewports =  create_info.iter().map(|info| {
            info.viewport_state.viewports.iter().map(|viewport|{
                vk::Viewport::builder()
                    .x(viewport.x)
                    .y(viewport.y)
        ...
        let scissors = create_info.iter().map(|info| {
            info.viewport_state.scissors.iter().map(|scissor|{
                vk::Rect2D::builder()
        ...
        let viewport_states = create_info.iter().enumerate().map(|(i, _info)|{
            vk::PipelineViewportStateCreateInfo::builder()
                .viewports(&viewports[i])
                .scissors(&scissors[i])
                .build()
        }).collect::<Vec<_>>();

        let rasterization_states = create_info.iter().map(|info|{
        ...
        let multisample_states = create_info.iter().map(|info|{
        ...
        let depth_stencil_states = create_info.iter().map(|info|{
            match info.depth_stencil_state {
                Some(state) => {
                    vk::PipelineDepthStencilStateCreateInfo::builder()
                        ...
                        .depth_compare_op(vk::CompareOp::from_raw(state.depth_compare_op.as_raw()))
        ...
        let attachments = create_info.iter().map(|info| {
            info.color_blend_state.attachments.iter().map(|attachment| {
        ...
        let color_blend_states = create_info.iter().enumerate().map(|(i, info)| {
        ...
        let dynamic_states_sets = create_info.iter().map(|info| {
            match info.dynamic_state {
                Some(dynamic_state) => {
                    dynamic_state.dynamic_states.iter().map(|dynamic_state| {
        ...
        let dynamic_states = create_info.iter().enumerate().map(|(i, info)| {
            match info.dynamic_state {
                Some(dynamic_state) => {
                    vk::PipelineDynamicStateCreateInfo::builder()
                        ...
                        .dynamic_states(&dynamic_states_sets[i])
                        .build()
                }
                None => {
                    vk::PipelineDynamicStateCreateInfo::builder()
        ...
        let pipeline_create_infos = create_info.iter().enumerate().map(|(i, info)|{
    ...
    fn create_render_pass(&self, create_info: &RHIRenderPassCreateInfo) -> Result<Box<dyn RHIRenderPass>> {
        ...
        let input_attachments = create_info.subpasses.iter().map(|subpass|{
            subpass.input_attachments.iter().map(|attachment|{
        ...
        let color_attachments = create_info.subpasses.iter().map(|subpass|{
            subpass.color_attachments.iter().map(|attachment|{
                vk::AttachmentReference::builder()
                    .attachment(attachment.attachment)
                    .layout(vk::ImageLayout::from_raw(attachment.layout.as_raw()))
                    .build()
        ...
        let resolve_attachments = create_info.subpasses.iter().map(|subpass|{
            subpass.resolve_attachments.iter().map(|attachment|{
                vk::AttachmentReference::builder()
                    .attachment(attachment.attachment)
                    .layout(vk::ImageLayout::from_raw(attachment.layout.as_raw()))
                    .build()
        ...
        let depth_stencil_attachments = create_info.subpasses.iter().map(|subpass|{
            vk::AttachmentReference::builder()
        ...
        let vk_subpasses = create_info.subpasses.iter().enumerate().map(|(i, subpass)| {
            vk::SubpassDescription::builder()
                ...
                .resolve_attachments(&resolve_attachments[i])
    ...
    fn update_descriptor_sets(&self, writes: &[RHIWriteDescriptorSet]) -> Result<()> {
        ...
        let image_infos = writes.iter().map(|write|{
            write.image_info.iter().map(|info| {
        ...
        let buffer_infos = writes.iter().map(|write|{
            write.buffer_info.iter().map(|info| {
        ...
        let texel_buffer_views = writes.iter().map(|write|{
            write.texel_buffer_view.iter().map(|view| {
                view.as_any().downcast_ref::<VulkanBufferView>().unwrap().get_resource()
            }).collect::<Vec<_>>()
        }).collect::<Vec<_>>();

        let vk_writes = writes.iter().enumerate().map(|(i, write)| {
            let vk_write = vk::WriteDescriptorSet::builder()
            ...
            if image_infos[i].len() > 0 {
                vk_write.image_info(&image_infos[i]).build()
            }
            else if buffer_infos[i].len() > 0 {
                vk_write.buffer_info(&buffer_infos[i]).build()
            }
            else if texel_buffer_views[i].len() > 0 {
                vk_write.texel_buffer_view(&texel_buffer_views[i]).build()
            }
            else {
                vk_write.build()
            }
    ...
    fn reset_command_pool(&self) -> Result<()> {
        unsafe {
            self.m_device.as_ref().unwrap().reset_command_pool(self.m_command_pools[self.m_current_frame_index], vk::CommandPoolResetFlags::empty())?;
        }
        Ok(())
    }
    ...
    fn wait_for_fence(&self) -> Result<()>{
        unsafe {
            self.m_device.as_ref().unwrap().wait_for_fences(&[self.m_is_frame_in_flight_fences[self.m_current_frame_index]], true, std::u64::MAX)?;
        }
        Ok(())
    }
    ...
    fn prepare_before_pass(&mut self, pass_update_after_recreate_swapchain: &dyn Fn()) -> Result<bool> {
        unsafe{
            let result = self.m_device.as_ref().unwrap().acquire_next_image_khr(
                self.m_swapchain, 
                u64::MAX,
                self.m_image_available_for_render_semaphores[self.m_current_frame_index], 
                vk::Fence::null(),
            );
            ...
            match result {
                ...
                Ok((image_index, vk::SuccessCode::SUBOPTIMAL_KHR)) => {
                    self.recreate_swapchain()?;
                    pass_update_after_recreate_swapchain();
                    self.m_current_swapchain_image_index = image_index;

                    let wait_stages = [vk::PipelineStageFlags::BOTTOM_OF_PIPE];
                    let submit_info = vk::SubmitInfo::builder()
                        .wait_semaphores(&[self.m_image_available_for_render_semaphores[self.m_current_frame_index]])
                        .wait_dst_stage_mask(&wait_stages)
                        .build(); 

                    self.m_device.as_ref().unwrap().reset_fences(&[self.m_is_frame_in_flight_fences[self.m_current_frame_index]])?;
                    self.m_device.as_ref().unwrap().queue_submit(
                        self.m_graphics_queue.as_ref().unwrap().as_any().downcast_ref::<VulkanQueue>().unwrap().get_resource(), 
                        &[submit_info], 
                        vk::Fence::null()
                    )?;
                    self.m_current_frame_index = ( self.m_current_frame_index + 1 ) % Self::K_MAX_FRAMES_IN_FLIGHT as usize;
                    return Ok(true);
                }
                Ok((image_index, _)) => {
                    self.m_current_swapchain_image_index = image_index;
            ...
            let command_buffer = &self.m_command_buffers.as_ref().unwrap()[self.m_current_frame_index];
            let command_buffer = command_buffer.as_any().downcast_ref::<VulkanCommandBuffer>().unwrap().get_resource();
            self.m_device.as_ref().unwrap().begin_command_buffer(command_buffer, &command_buffer_begin_info)?;
        }
    ...
    fn submit_rendering(&mut self, pass_update_after_recreate_swapchain: &dyn Fn()) -> Result<()> {
        ...
        let command_buffer = command_buffer.as_any().downcast_ref::<VulkanCommandBuffer>().unwrap().get_resource();
        unsafe{
            self.m_device.as_ref().unwrap().end_command_buffer(command_buffer)?;
        }

        // let semphore = &self.m_image_available_for_texturescopy_semaphores.as_ref().unwrap()[self.m_current_frame_index];
        // let semphore = semphore.as_any().downcast_ref::<VulkanSemaphore>().unwrap().get_resource();

        let semphores = [
            // semphore,
            self.m_image_finished_for_presentation_semaphores[self.m_current_frame_index],
        ];
        ...
        unsafe{
            ...
            self.m_device.as_ref().unwrap().queue_submit(
                self.m_graphics_queue.as_ref().unwrap().as_any().downcast_ref::<VulkanQueue>().unwrap().get_resource(), 
                &[submit_info], 
                self.m_is_frame_in_flight_fences[self.m_current_frame_index]
            )?;

            let present_info = vk::PresentInfoKHR::builder()
                .wait_semaphores(&[self.m_image_finished_for_presentation_semaphores[self.m_current_frame_index]])
                .swapchains(&[self.m_swapchain])
                .image_indices(&[self.m_current_swapchain_image_index])
                .build();

            let result = self.m_device.as_ref().unwrap().queue_present_khr(
                self.m_present_queue, 
                &present_info
            );
...
impl VulkanRHI {
    ...
    fn create_instance(&mut self) -> Result<()> {
        ...
        debug_info = Self::populate_debug_messenger_info(debug_info);
    ...
    fn create_command_buffers(&mut self) -> Result<()> {
        ...
        self.m_command_buffers = Some(array::from_fn(|i| {
            let allocate_info = allocate_info.command_pool(self.m_command_pools[i]);
