...
impl DebugDrawFont {
    ...
    pub fn create(rhi: &VulkanRHI) -> Result<Self> {
    ...
    fn load_font(&mut self, rhi: &VulkanRHI) -> Result<()> {
        let mut image_data = Vec::<f32>::new();
        image_data.resize((Self::BITMAP_W*Self::BITMAP_H) as usize, 0.0);

        let font_buffer = include_bytes!("../../../../editor/resource/PiccoloEditorFont.TTF");
        let face = ab_glyph::FontArc::try_from_slice(font_buffer).unwrap();
